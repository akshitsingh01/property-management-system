...
const myProperties = asyncHandler(async (req, res, next) => {
  ...
  const myProperties = await Property.find({postedBy : req?.user?._id}).populate('postedBy') ; ;
  if(!myProperties) 
  {
    throw new ApiError(500 , 'Something went wrong'  ) ;
  }
  return res.status(200).json(new ApiResponse(200 , myProperties , {message : "my properties"})) ;

});

export { registerProperty, getAllProperty, getSingleProperty, uploadPhotos , myProperties };
